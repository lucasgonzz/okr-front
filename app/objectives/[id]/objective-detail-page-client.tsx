...
function useObjectiveId(serverId: string): string {
  const pathname = usePathname();
  const params = useParams();
  const fromPath = pathname?.match(/^\/objectives\/([^/]+)/)?.[1];
  const raw = params?.id;
  const fromParams =
    typeof raw === "string" ? raw : Array.isArray(raw) ? raw[0] : undefined;

  for (const candidate of [fromPath, fromParams, serverId]) {
    if (candidate && candidate !== EXPORT_BUILD_PLACEHOLDER_ID) {
      return String(candidate);
    }
  }
  return String(fromPath ?? fromParams ?? serverId);
}
...
export function ObjectiveDetailPageClient({ id: serverId }: { id: string }) {
  ...
  const objectiveFromList = objectives.find((o) => String(o.id) === String(id));
  const listObjectiveId = objectiveFromList?.id;

  const loadDetail = useCallback(async () => {
    try {
      const res = await apiFetch<{ objective: Objective }>(`/objectives/${id}`);
      setDetail(res.objective ?? null);
    } catch {
      setDetail(null);
    }
  }, [id]);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const res = await apiFetch<{ objective: Objective }>(`/objectives/${id}`);
        if (!cancelled) setDetail(res.objective ?? null);
      } catch {
  ...
  }, [id, listObjectiveId, refreshObjectives]);
  ...
  const applyObjectiveUpdate = useCallback((updated: Objective | null) => {
    if (updated) {
      setDetail(updated);
    }
  }, []);
  ...
  const confirmDeleteKr = async () => {
    if (krToDelete && objective) {
      ...
      try {
        const updated = await deleteKeyResult(objective.id, krToDelete.id);
        applyObjectiveUpdate(updated);
        if (!updated) await loadDetail();
        setKrToDelete(null);
  ...
  const handleQuickUpdate = async (krId: string, currentValue: number) => {
    ...
    try {
      const updated = await updateKeyResult(objective.id, krId, { currentValue, progress, status });
      applyObjectiveUpdate(updated);
      if (!updated) await loadDetail();
    } finally {
    ...
    return (
      ...
      >
        ...
        >
          ...
          <p className="mt-4 text-sm text-muted-foreground">Cargando objetivo...</p>
        </motion.div>
      </AppShell>
    );
  }
